...
\begin{itemize}
	...
	\item \textbf{faster\_ramp\_up} : (Integer) Enables different ramp up modes, if 0 the standard one is used, i.e. $antenna\_factor = antenna_\_factor_0 + time^2$. If 1, the antenna factor reaches its experimental value at \texttt{t\_max\_ramp\_up}
	...
	\item \textbf{temperature\_limit} : (Double) Lower electron and ion temperature limit in eV. Simulations seem to be sensitive to this value. If it is bad, the calculation will get stuck in a permanent loop. Usually, the value is about $10$ to $50eV$. Best way to check if the code got stuck is checking the size of the output hdf5 file. It should increase at least every minute.
