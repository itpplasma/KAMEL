...
contains
    ...
    subroutine display_kim_configuration()
        ...
        if (trim(type_of_run) == 'electrostatic_periodic') then
            ...
            call print_bool_line('Periodic Ion Tensor', &
                periodic_calculate_ion_tensor, width)
        end if
        ...
        write(value_str, '(I0)') mphi_max
        call print_config_line('Max. cyclotron harmonics', trim(value_str), width)
