...
END MODULE nrutil
! Achtung
! bessj, bessj0, bessj1 - double precision hinzugef�gt WiKe
! bessk, bessk0, bessk1 - double precision hinzugef�gt WiKe
! bessi, bessi0, bessi1 - double precision hinzugef�gt WiKe
...
!BL
		FUNCTION bessi_d(n,x)
		USE nrtype
		INTEGER(I4B), INTENT(IN) :: n
		REAL(DP), INTENT(IN) :: x
		REAL(DP) :: bessi_d
		END FUNCTION bessi_d
!BL
...
!BL
		FUNCTION bessk_d(n,x)
		USE nrtype
		INTEGER(I4B), INTENT(IN) :: n
		REAL(DP), INTENT(IN) :: x
		REAL(DP) :: bessk_d
		END FUNCTION bessk_d
!BL
...
!BL
	...
	END INTERFACE
END MODULE nr
! Achtung bessk0_d hinzugef�gt WiKe
!
!
! ACHTUNG: Routine liefert K_n(x)*SQRT(X)*EXP(X)
!
	FUNCTION bessk0_s(x)
	...
	IF (x <= 2.0) THEN
		...
		bessk0_s=bessk0_s*SQRT(x)*EXP(x)
	ELSE
		y=(2.0_sp/x)
!		bessk0_s=(EXP(-x)/SQRT(x))*poly(y,q)
	...
	REAL(DP), DIMENSION(7) :: q = (/1.25331414_dp,-0.7832358e-1_dp,&
		0.2189568e-1_dp,-0.1062446e-1_dp,0.587872e-2_dp,&
		-0.251540e-2_dp,0.53208e-3_dp/)
	CALL assert(x > 0.0, 'bessk0_d arg')
	IF (x <= 2.0) THEN
		y=x*x/4.0_dp
		bessk0_d=(-LOG(x/2.0_dp)*bessi0(x))+poly(y,p)
		bessk0_d=bessk0_d*SQRT(x)*EXP(x)
	ELSE
		y=(2.0_dp/x)
!		bessk0_d=(EXP(-x)/SQRT(x))*poly(y,q)
		bessk0_d=poly(y,q)
	END IF
	END FUNCTION bessk0_d
	...
	WHERE (mask)
		...
		bessk0_v=bessk0_v*SQRT(x)*EXP(x)
	ELSEWHERE
		y=(2.0_sp/x)
!		bessk0_v=(EXP(-x)/SQRT(x))*poly(y,q,.NOT. mask)
		bessk0_v=poly(y,q,.NOT. mask)
	END WHERE
	END FUNCTION bessk0_v
! Achtung bessk1_d hinzugef�gt WiKe
!
!
! ACHTUNG: Routine liefert K_n(x)*SQRT(X)*EXP(X)
!
	...
	IF (x <= 2.0) THEN
		...
		bessk1_s=(LOG(x/2.0_sp)*bessi1(x))+(1.0_sp/x)*poly(y,p)
                bessk1_s=bessk1_s*SQRT(x)*EXP(x)
	ELSE
		y=2.0_sp/x
!		bessk1_s=(EXP(-x)/SQRT(x))*poly(y,q)
	...
	REAL(DP), DIMENSION(7) :: q = (/1.25331414_dp,0.23498619_dp,&
		-0.3655620e-1_dp,0.1504268e-1_dp,-0.780353e-2_dp,&
		0.325614e-2_dp,-0.68245e-3_dp/)
	CALL assert(x > 0.0, 'bessk1_d arg')
	IF (x <= 2.0) THEN
		y=x*x/4.0_dp
		bessk1_d=(LOG(x/2.0_dp)*bessi1(x))+(1.0_dp/x)*poly(y,p)
                bessk1_d=bessk1_d*SQRT(x)*EXP(x)
	ELSE
		y=2.0_dp/x
!		bessk1_d=(EXP(-x)/SQRT(x))*poly(y,q)
		bessk1_d=poly(y,q)
	END IF
	END FUNCTION bessk1_d
	...
	WHERE (mask)
		...
		bessk1_v=(LOG(x/2.0_sp)*bessi1(x))+(1.0_sp/x)*poly(y,p,mask)
                bessk1_v=bessk1_v*SQRT(x)*EXP(x)
	ELSEWHERE
		y=2.0_sp/x
!		bessk1_v=(EXP(-x)/SQRT(x))*poly(y,q,.NOT. mask)
		bessk1_v=poly(y,q,.NOT. mask)
	END WHERE
	END FUNCTION bessk1_v
! Achtung bessk_d hinzugef�gt WiKe
!
! ACHTUNG: Routine liefert K_n(x)*SQRT(X)*EXP(X)
!
!
	...
	CALL assert(n >= 2, x > 0.0, 'bessk_d args')
	tox=2.0_dp/x
	bkm=bessk0(x)
	bk=bessk1(x)
	DO j=1,n-1
	...
	bessk_v=bk
	END FUNCTION bessk_v
! Achtung bessi0_d eingef�gt WiKe
!
        FUNCTION bessi0_s(x)
	...
	ax=ABS(x)
	IF (ax < 3.75) THEN
		bessi0_d=poly(REAL((x/3.75_dp)**2,dp),p)
	ELSE
		bessi0_d=(EXP(ax)/SQRT(ax))*poly(REAL(3.75_dp/ax,dp),q)
	END IF
	END FUNCTION bessi0_d
	...
	END WHERE
	END FUNCTION bessi0_v
! Achtung bessi1_d eingef�gt WiKe
!
	FUNCTION bessi1_s(x)
	USE nrtype; USE nrutil, ONLY : poly
	IMPLICIT NONE
	...
	ax=ABS(x)
	IF (ax < 3.75) THEN
		bessi1_d=ax*poly(REAL((x/3.75_dp)**2,dp),p)
	ELSE
		bessi1_d=(EXP(ax)/SQRT(ax))*poly(REAL(3.75_dp/ax,dp),q)
	END IF
	IF (x < 0.0) bessi1_d=-bessi1_d
	END FUNCTION bessi1_d
	...
	END FUNCTION bessi1_v
! Achtung bessi_d eingef�gt WiKe
!
	FUNCTION bessi_s(n,x)
	USE nrtype; USE nrutil, ONLY : assert
	USE nr, ONLY : bessi0
	...
	DO j=m,1,-1
		...
		bi=bim
		IF (EXPONENT(bi) > IEXP) THEN
			bessi_d=SCALE(bessi_d,-IEXP)
			bi=SCALE(bi,-IEXP)
			bip=SCALE(bip,-IEXP)
		END IF
		IF (j == n) bessi_d=bip
	END DO
	bessi_d=bessi_d*bessi0(x)/bi
	IF (x < 0.0 .AND. MOD(n,2) == 1) bessi_d=-bessi_d
	END FUNCTION bessi_d
	...
	END FUNCTION bessi_v
!
  subroutine macdonald(n_in,x,bk,dbk)
!
  use nr, only : bessk,bessk0,bessk1
!
  implicit none
!
  integer :: n_in,n
  double precision :: x,bk,dbk,yp,ym
!
  n=abs(n_in)
!
  if(n.gt.1) then
    bk=bessk(n,x)*exp(-x)/sqrt(x)
  elseif(n.eq.1) then
    bk=bessk1(x)*exp(-x)/sqrt(x)
  elseif(n.eq.0) then
    bk=bessk0(x)*exp(-x)/sqrt(x)
  endif
!
  if(n.gt.2) then
    ym=bessk(n,x)*exp(-x)/sqrt(x)
  elseif(n.eq.2.or.n.eq.0) then
    ym=bessk1(x)*exp(-x)/sqrt(x)
  elseif(n.eq.1) then
    ym=bessk0(x)*exp(-x)/sqrt(x)
  endif
  dbk=-dfloat(n)*bk/x-ym
!
  end subroutine macdonald
