program test_bparallel_linear_response
    ...
    use rt_electrostatic_periodic_m, only: PERIODIC_BPAR_INVALID_DRIVE
...
contains
    ...
    subroutine test_support_validation()
        ...
        invalid_drive = cmplx(ieee_value(0.0_dp, ieee_quiet_nan), 0.0_dp, dp)
        call require_status(periodic_bparallel_support_status(invalid_drive, &
            'FokkerPlanck', 'FokkerPlanck', .false., 0, 0, .false.), &
            PERIODIC_BPAR_INVALID_DRIVE, 'non-finite Bparallel drive rejected')
