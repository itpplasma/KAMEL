program test_dqli_limit_benchmark
    !! Algebraic limiting-case benchmark for the finite-Larmor-radius ion tensor.
    !! At ell=0, B_parallel=0, and k_perp rho_i -> 0 the FLR expression
    !! must reduce to the established Heyn/Markl tensor.
    ...
    real(dp) :: flr_tensor(2,2), drift_tensor(2,2)
    ...
    do ik = 1, size(ks_values)
        ...
        call calc_ion_flr_harmonic(0, ks, 0.0_dp, ks, 0.0_dp, vT, omega_c, -omega_c, &
            c_light, B0, nu, fields, fields, ifunc, flr_tensor)
        scale = max(1.0_dp, maxval(abs(drift_tensor)))
        limit_error = maxval(abs(flr_tensor-drift_tensor))/scale
        if (ik == 1 .and. limit_error > 2.0e-12_dp) &
            then
            print *, 'FLR tensor = ', flr_tensor
            print *, 'drift tensor    = ', drift_tensor
            print *, 'relative error  = ', limit_error
            error stop 'exact zero-FLR-to-drift benchmark mismatch'
        end if
        if (ik == 2 .and. limit_error > 2.0e-7_dp) &
            error stop 'finite small-FLR-to-drift convergence mismatch'
    end do
...
contains
    ...
    subroutine test_public_benchmark_api()
        ...
        ! rather than compare a magnetic-only FLR tensor with an
