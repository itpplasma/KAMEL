program test_periodic_config_read
    ...
    use config_m, only: nml_config_path, periodic_Bparallel_drive, &
        periodic_dr_asis_scale, periodic_dr_tr_scale, periodic_kmax_scale, &
        periodic_n_rg

    implicit none

    complex(dp), parameter :: expected_drive = cmplx(0.25_dp, -0.5_dp, dp)
    ...
    ! Rejection fixtures are registered with WILL_FAIL. Returning from the
    ! production parser therefore intentionally means test failure.
    call kim_read_config
    if (.not. should_reject .and. &
            periodic_Bparallel_drive /= expected_drive) then
        error stop 'valid KIM_PERIODIC drive was not read exactly'
    end if
    if (defaults_reset_case) then
        ...
        if (periodic_dr_asis_scale /= 5.0_dp .or. &
                ...
                periodic_Bparallel_drive /= (0.0_dp, 0.0_dp)) then
