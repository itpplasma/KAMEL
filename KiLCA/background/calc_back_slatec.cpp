//This old version of the code uses slatec for background ODE solution
...
void deriv_eqn_equil (double *x, double *u, double *du, double *rtor, int *bpro)
{
background *bp = (background *)(*bpro);

spline_eval_ (bp->sid, 1, x, 0, 1, bp->i_q, bp->i_T[1], bp->R);

//R[n-Dmin+(j-Imin)*D1+k*D2], k=0 - new
...
double g = 1.0+(*x)*(*x)/(*rtor)/(*rtor)/q/q;

du[0] = - 2.0*(*x)*u[0]/(q*q*g*(*rtor)*(*rtor)) - 8.0*pi*dpress;
}
...
{
/*calculates magnetic field and plasma current out of the input profiles*/

/*solver settings:*/
double rtol = 1.0e-12, atol = 1.0e-12;
const int neq = 1, lrw = 151, liw = 51;
int info[15], iwork[liw];
double rwork[lrw];
int bpro, idid;
...
spline_eval_ (sid, 1, &r0, 0, 0, i_q, i_q, R);

/*calculation of u function:*/
double g = 1.0 + r0*r0/rtor/rtor/(R[0])/(R[0]);
...
for (i=1; i<dimx; i++)
{
    ...
    /*warning: slatec subs are not threads safe!!!*/
    ddeabm_ (deriv_eqn_equil, &neq, &r0, &uval, &r1, info, &rtol, &atol,
             &idid, rwork, &lrw, iwork, &liw, &rtor, &bpro);
...
#if USE_ANOMALOUS_FREQ == YES

double q[2];

double kz = 3.0/(sd->bs->rtor), fac; //n = 3
double Dperp = 1.0e4;
...
for (i=0; i<dimx; i++)
{
    ...
    spline_eval_ (sid, 1, x+i, 0, 1, i_q, i_q, q);

    fac = pow(pow(kz*(q[1]/q[0]), 2.0)*Dperp, 1.0/3.0);

    nui[i] += fac * pow(boltz*Ti[i]/mass[0], 1.0/3.0);
    nue[i] += fac * pow(boltz*Te[i]/mass[1], 1.0/3.0);
...
for (i=0; i<dimx; i++)
{
    eval_and_set_background_parameters_spec_independent_ (x+i, flag_back);

    for (spec=0; spec<2; spec++)
    {
        eval_and_set_background_parameters_spec_dependent_ (x+i, &spec, flag_back);
        eval_and_set_f0_parameters_nu_and_derivs_ (x+i, &spec, flag_back);
