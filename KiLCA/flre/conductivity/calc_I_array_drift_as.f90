...
integer, parameter :: Nc = 4, deg = 7, nmax = 3 * (deg + 1) * 6;
...
real(dp) :: pow, pow_min, pow_max, pow_del;

real(dp), dimension(1:nmax) :: taut, argt;
...
pow_min = -(deg + 1);
pow_max =  (deg + 1) * 2.0d0;
pow_del = (pow_max - pow_min) / (nmax - 1);
...
do k = 1,Nc
    ...
    do while (n < nmax)
        ...
        pow = pow_min + pow_del * (n - 1);

        tmax_est = 2.0d0**pow * (argmin / real(gr * dtaudt));
        ...
        if (argt(n) < argmin .and. pow > 0) exit;
...
use spec_data, only: argmin, dtaudt;
use spec_data, only: feval;
...
!dtaudt factor is probably missing!!!
