...
allocate(dblfac(-1:2*mpara), binocoef(0:mpara, 0:mpara), pow2(0:2*mpara), powmibx1(0:2*mpara));
...
real(dp), dimension(-deg:deg)  :: taut, argt;
...
do k = 0,Nc-1
    ...
    tmax_est = 2.0d0**(-deg-1) * (argmin / real(gr * dtaudt));
...
do n = deg-1,-deg,-1
    ...
    if ( (argt(n+1) - argmin) < 0.0d0 .and. (argt(n) - argmin) > 0.0d0 ) then

        z_start(1) = (taut(n) + taut(n+1)) / 2.0d0;
        zmin = taut(n);
        zmax = taut(n+1);
        exit;
...
if(ier /= 0) then
    ...
    print *, 'error: solve_system_newton: failed to find taumax!';
...
use spec_data, only: bx1, bx2, bx4, gamma, gr, powmibx1, dtaudt, argmin;
...
complex(dpc), dimension(0:npar,0:npar,1:nper) :: Imnl;
integer :: status;
...
if (status /= 0) then
    ...
    print *, 'error: calc_imnl_quad: failed to find the optimal contour!';
    return;

end if

! if (-real(dtaudt)*TOUT < argmin) then ! asymptotic expression
!
!     taumax = - argmin / real(dtaudt);
!
!     call asymptotic(dtaudt*taumax, Imnl, status);
!
!     if (status /= 0) then
!
!         print *, 'warning: calc_imnl_quad: the asymptotic expression failed: switched to direct integration.';
!
!         Imnl = cmplx(0.0d0, 0.0d0, dp);
!
!     else
...
do mm = 0,npara
    ...
    do nn = 0,mm
        ...
        do ll = 1,nperp
            ...
            Imnl(mm,nn,ll) = factor * (III(ind) + Imnl(mm,nn,ll));
...
subroutine asymptotic(tau, Imnl, status)

use constants, only: dpc, im;
use spec_data, only: npara, nperp;
use spec_data, only: bx1, bx4, gamma, gr;
use spec_data, only: MC, powmibx1, arrfac3, arrfacl;

implicit none;

complex(dpc) :: tau;
complex(dpc), dimension(0:npara,0:npara,1:nperp) :: Imnl;
integer :: status;

complex(dpc), parameter :: one = cmplx(1.0d0, 0.0d0, dpc);

complex(dpc) :: fac, comm, summa;
...
z = - gr * (one + im * bx4 * tau) / im / bx4;

do ll = 1,nperp

    s = cmplx((1.0d0 - dble(ll))/2.0d0, 0.0d0, dpc);

    call igamma(s, z, abs_err, rel_err, scal, expigamma, est_err, Nterms, status);

    if (status /= 0) then

        print *, 'warning: incomplete_gamma: poor convergence:', s, z, expigamma, est_err, Nterms, status;

    end if

    arrfacl(ll) = ( (-gr)**(dble(ll-1)/2.0d0) / (im * bx4)**(dble(ll+1)/2.0d0) ) * expigamma;

end do

comm = one / sqrt(one - gamma*gamma) * exp( gr * tau + (one - 2.0d0 * gamma) / (one - gamma) * bx1 * bx1 );
    ...
    do nn = 0,mm
        ...
        do j1 = 0,jmax
            ...
            do j2 = 0,jmax
                ...
                summa = summa + MC(mm,nn,j1,j2) * powmibx1(ind) * arrfac3(mm + nn - j1 - j2);
        ...
        do ll = 1,nperp
            ...
            Imnl(mm, nn, ll) = comm * arrfacl(ll) * summa;
