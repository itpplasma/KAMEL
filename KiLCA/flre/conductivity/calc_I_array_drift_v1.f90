...
integer, parameter             :: n_start = 2*deg+1;
real(dp), dimension(1:n_start) :: z_start;
real(dp)                       :: abs_eps_z = 1.0d-3, rel_eps_z = 1.0d-3;
real(dp)                       :: abs_eps_f = 1.0d0,  rel_eps_f = 1.0d-1;
integer                        :: ier, max_iter_num = 32;
real(dp)                       :: rest;

real(dp) :: der, tmax_est, arg0, argt;
...
do k = 0,Nc-1
    ...
    do n = -deg,deg
        ...
        argt = log(maxval(abs(III)));

        !print *, 'info: find_opt_contour: ', 'phit =', phit, 'grt =', real(gr*dtaudt), 'tmax_est =', tmax_est, 'argt =', argt;

        if(argt > 0.0d0 .and. argt > min(1.2d0 * arg0, arg0 + 1.0d0)) then
...
if(phi == pi) then
    ...
    print *, 'warning: find_opt_contour: unable to match all requirements for the contour:', gr, phi1, phi2, phi;
...
! find tmax:
tmax_est = 2.0d0**(deg) * (argmin / real(gr * dtaudt));

do n = 1,n_start

    z_start(n) = tmax_est;

    tmax_est = tmax_est / 2.0d0;
...
if(ier /= 0) then
    ...
    status = 1;
    print *, 'error: solve_system_newton: failed to find taumax:', 'phio =', phi, 'gro =', gro, 'tmax =', tmax, 'rest =', rest;
...
do while(IDID < 2)
    ...
    if (IDID < -1) then
        print *, 'error: calc_imnl_quad: solver failed to integrate the equations:', TOUT, T, IDID;
        status = 1;
        exit;
    end if
