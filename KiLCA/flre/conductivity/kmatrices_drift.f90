...
use conduct_arrays, only: Imnkl, D;
use flre_sett, only: Nmax, flre_order, Nbmax, rsp;
...
sum_int = sum_int + 2.0d0**(FI(munu,2) - abs(l) - b1 - b2) * (-1.0d0)**(b1 + b2) * &
          ...
          (Q(al,0,0) * Imnkl(0, FI(munu,1), ki, li, l) + &

           Q(al,1,0) * Imnkl(1, FI(munu,1), ki, li, l) + &

           Q(al,2,0) * Imnkl(2, FI(munu,1), ki, li, l) + &

           Q(al,0,2) * Imnkl(0, FI(munu,1), ki + 2, li, l) * 2.0d0 * (FI(munu,2) + abs(l) + b1 + b2 + 1));
