...
!
  ...
  double complex, dimension(0:mpar,0:mpar,0:mperp,0:mperp) :: Imnkl
  double complex, dimension(0:mpar,0:mpar,0:mperp,0:mperp) :: Imnkl_I

  integer :: lperp, status
...
!
  call getIfunc_drift(consenergy,mpar,mperp,x1,x2,x3,x4,Imnkl)
!
write(777,*) r, x1, x2, x3, x4, real(Imnkl), imag(Imnkl)

lperp = 2*mperp + 1

call init_spec_data(mpar, lperp);

x1c = x1; x2c = x2; x3c = x3; x4c = x4;

call calc_imnkl_quad(consenergy, mpar, mperp, x1c, x2c, x3c, x4c, Imnkl_I, status)

call close_spec_data();

write(888,*) r, x1, x2, x3, x4, real(Imnkl_I), imag(Imnkl_I)

write(999,*) r, x1, x2, x3, x4, maxval(abs((Imnkl_I - Imnkl)/Imnkl))
