...
for(int l = 0; l <= *l_max; ++l)
{
    ...
    while(t2 <= tmax)
    {
        double I_re;
        qp.part = 0;
        //quad_qagiu(&subint, &qp, 0.0e0, epsabs, epsrel, limit, &I_re, &err);
        quad_qag(&subint, &qp, t1, t2, epsabs, epsrel, limit, 21, &I_re, &err);

        double I_im;
        qp.part = 1;
        //quad_qagiu(&subint, &qp, 0.0e0, epsabs, epsrel, limit, &I_im, &err);
        quad_qag(&subint, &qp, t1, t2, epsabs, epsrel, limit, 21, &I_im, &err);
