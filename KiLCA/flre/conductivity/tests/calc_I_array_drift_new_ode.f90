...
allocate(arrfac1(0:mpara), arrfac2(0:mpara), arrfac3(0:2*mpara), arrfacl(0:mperp));
...
! for ode solver:
MAXNEQ = (mperp + 1) * (mpara + 1) * (mpara + 2); ! max number of real equations
...
use spec_data, only: bx1, bx2, gr, phi, dtaudt, argmin;
...
do k = 0,Nc-1
    ...
    if(real(gr*dtaudt) >= 0.0d0) print *, 'IMPOSSIBLE: grt>0:', phit, real(gr * dtaudt);

    call jacob(0.0d0, dargdt);

    !print *, 'info: find_opt_contour: ', 'phit =', phit, 'grt =', real(gr*dtaudt), 'dargdt =', dargdt;

    if(dargdt <= 0.0d0) then
...
if(phi == - pi) then
    ...
    !print *, 'WARNING: find_opt_contour: failed to find a good contour: ', gr, phi1, phi2;
...
if(ier /= 0) then
    ...
    status = 1;
    print *, 'WARNING: solve_system_newton: failed to find taumax:', 'phio =', phi, 'gro =', gro, 'tmax =', tmax, 'rest =', rest;
...
complex(dpc) :: tau, expon, onepluexp, oneminexp, denplu, denmin, fac1, fac2, fac3, facm, facl, comm, summa;
...
do mm = 0,npara
    ...
    do nn = 0,mm
        ...
        do ll = 0,nperp
            ...
            ind = (mm - nn) + (nn * (2 * npara + 3 - nn)) / 2 + ll * (((npara + 1) * (npara + 2)) / 2);
...
complex(dpc), dimension(0:npar,0:npar,0:nper) :: Imnl;
integer :: status;
...
if(npar > mpara .or. nper > mperp) then
    ...
    print *, 'WARNING: the values of (npar, nper) do not match to (mpara, mperp):', npar, nper, mpara, mperp;
    status = 1;
    return;
...
NEQ = (nperp + 1) * (npara + 1) * (npara + 2); ! number of real equations

call find_opt_contour(TOUT, status);

if (status /= 0) print *, 'WARNING: failed to find contour';
...
do while(IDID < 2)
    ...
    if (IDID < -1) then
        print *, 'WARNING: solver failed to integrate the equations:', TOUT, T, IDID;
        status = 1;
        exit;
    end if
