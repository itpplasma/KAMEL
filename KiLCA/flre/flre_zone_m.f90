...
contains
    ...
    subroutine flre_calculate_field_profiles_orth(self)
        ...
        ! Pass contiguous columns by sequence association. This avoids invalid bounds
        ! checks generated by gfortran 16 for slices of polymorphic array components.
        k = 0
        j = j1
        do while ((j - j2)*dj <= 0)
            k = k + 1
    ...
    subroutine flre_calc_final_fields(self)
        ...
        do k = 1, dim_old
            call system_to_state_copy(self, self%EB(1, k), state_tmp)
            do i = 1, self%Nwaves
