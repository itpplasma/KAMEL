...
! conductivity matrices for all modes:
...
! for each mode k conductivity matrices, kmat(k), have the layout:
! (node=1:rdim, part={re,im}=1:2, j=1:3, i=1:3, n'=1:flre_order+1, n=1:flre_order+1)
type(pointer6D), allocatable, dimension(:) :: k_mat_i; ! (mode=1:dim_mn)
type(pointer6D), allocatable, dimension(:) :: k_mat_e; ! (mode=1:dim_mn)

! correction matrices:
type(pointer4D), allocatable, dimension(:) :: c_mat_i; ! (mode=1:dim_mn)
type(pointer4D), allocatable, dimension(:) :: c_mat_e; ! (mode=1:dim_mn)

! for each mode k, conductivity radial grid, rgrd(k), has the layout:
! (node=1:rdim)
type(pointer1D), allocatable, dimension(:) :: rgrd; ! (mode=1:dim_mn)

! for each mode k, dimension of conductivity radial grid, rdim(k):
integer, allocatable, dimension(:) :: rdim; ! (mode=1:dim_mn)

integer :: flreo; ! order of FLR expansion
...
type(c_ptr) :: kptr, rptr;

integer :: zone_ind = 0, spec;
...
do k = imin,imax
    ...
    ! main conductivity matrices:
    call calc_conductivity_matrices_for_mode(flre_cd_ptr(k), flre_path, len(trim(flre_path)), m_vals(k), n_vals(k));

    spec = 0;
    call get_kilca_conductivity_array(flre_cd_ptr(k), m_vals(k), n_vals(k), zone_ind, spec, flreo, rdim(k), rptr, kptr);
    call c_f_pointer(kptr, k_mat_i(k)%a, [rdim(k), 2, 3, 3, flreo+1, flreo+1]);

    spec = 1;
    call get_kilca_conductivity_array(flre_cd_ptr(k), m_vals(k), n_vals(k), zone_ind, spec, flreo, rdim(k), rptr, kptr);
    call c_f_pointer(kptr, k_mat_e(k)%a, [rdim(k), 2, 3, 3, flreo+1, flreo+1]);

    call c_f_pointer(rptr, rgrd(k)%a, [rdim(k)]);

    ! conductivity correction matrices:
    allocate(c_mat_i(k)%a(1:3,1:3,0:1,1:rdim(k)), c_mat_e(k)%a(1:3,1:3,0:1,1:rdim(k)));

    call set_wave_parameters(flre_cd_ptr(k), m_vals(k), n_vals(k));

    spec = 0; call calc_correction_matrices_in_cylindrical_system(rdim(k), rgrd(k)%a, spec, c_mat_i(k)%a);
    spec = 1; call calc_correction_matrices_in_cylindrical_system(rdim(k), rgrd(k)%a, spec, c_mat_e(k)%a);

    call unset_wave_parameters();
...
allocate(k_mat_i(dim_mn), k_mat_e(dim_mn), c_mat_i(dim_mn), c_mat_e(dim_mn), rgrd(dim_mn), rdim(dim_mn));
...
do k = 1,dim_mn
    if (associated(c_mat_i(k)%a)) deallocate(c_mat_i(k)%a);
    if (associated(c_mat_e(k)%a)) deallocate(c_mat_e(k)%a);
end do

deallocate(k_mat_i, k_mat_e, c_mat_i, c_mat_e, rgrd, rdim);
...
integer :: deg = 15, Dmin = 0, Dmax = 0, l, ind;
