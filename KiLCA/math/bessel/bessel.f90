...
if(n == 0) then
    call zbesj (dble(zarg), dimag(zarg), abs(dble(nu)), 1, 1, rr, ri, nz, ierr)
    ...
    if (ierr > 0 .and. ierr /= 3) then
        print *, 'besselj: failed to compute besselj, ierr=',ierr, '. Answer is set to unit!'
        rr = 1.0d0
        ri = 0.0d0
    end if
...
if (allocated(bico)) deallocate (bico);
