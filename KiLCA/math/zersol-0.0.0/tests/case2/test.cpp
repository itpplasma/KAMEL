...
{
return z * z * (z-2.0) * (z-2.0) * (std::exp(2.0*z) * std::cos(z) + z * z * z - 1.0 - std::sin(z));
}
...
{
type::Function<double> F(f);                  // define the function object with numerical evaluation of the derivative
...
//type::Box<double> B(-1.0, 4.0, -1.0, 1.0);  // Attention: so defined box will cause the solver error since the zeros
                                              ...
                                              // we recommend to run this case to get familar with the situation.

type::Box<double> B(-1.0, 4.0, -1.0, 2.0);    // define the rectangular region (box) in the complex plane;
                                              // the partitioning of this box will not cross the function zeros;
                                              // run this case to get the function zeros
