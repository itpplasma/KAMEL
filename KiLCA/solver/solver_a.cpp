...
/* initial values: */
*rdata = rvec[0];
for (i=0; i<Neq; i++) ydata[i] = Smat[i];
...
while(1)
{
    ...
    if (max > (ss->norm_fac)*min) ort_flag = 0; else ort_flag = 1;

    if (ort_flag == 1) continue; /*ydata and rdata are inchanged, just next step:*/
    ...
    for (i=0; i<Neq; i++) ydata[i]=adr[i]; /*stores the ort. data*/
...
for (k=dim-1; k>-1; k--) /* index of r grid point */
{
	...
	{
		if (dirint*(rvec[k]-(*rdata))>0.0) break;
