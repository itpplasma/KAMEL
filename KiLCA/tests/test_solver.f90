module solver_test_rhs_m
    ...
    implicit none
contains
    subroutine diagonal_rhs(t, y, dy, params) bind(C)
...
program test_solver
    ...
    do mode = 1, 2
        ...
        state(1, 1) = 1d0
        state(7, 1) = 1d0
        rc = integrate_basis_vecs(c_funloc(diagonal_rhs), 2_c_int, 2_c_int, 5_c_int, &
        ...
        do k = 1, 5
            expected = 0d0
            expected(1) = state(1, 5) * exp(grid(k) - grid(5))
            expected(7) = state(7, 5) * exp(grid(5) - grid(k))
