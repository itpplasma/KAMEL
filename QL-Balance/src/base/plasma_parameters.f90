...
module plasma_parameters
    ...
    !> @details ddr_params: gradients from actual plasma state
    !>          ddr_params_lin: gradients from linearized state (for Jacobian probing)
    !>          Same units as params but per cm (e.g., [cm^-3/cm] = [cm^-4])
    real(dp), dimension(:, :), allocatable :: ddr_params, ddr_params_lin
