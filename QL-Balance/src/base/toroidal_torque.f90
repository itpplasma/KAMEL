subroutine calculate_total_toroidal_torque(time_index)
    ...
    use grid_mod, only: T_EM_phi_e, T_EM_phi_i, torque_ntv, T_tot_phi_e, T_tot_phi_i, T_tot_ntv_i
    ...
    call simpson_nonequi(T_tot_ntv_i(time_index), r, r * torque_ntv)
    ...
    T_tot_ntv_i(time_index) = T_tot_ntv_i(time_index) * 4.0_dp * pi**2 * rtor
...
subroutine write_total_toroidal_torque_to_file(time_index)
    ...
    if (time_index == 1) then
        ...
        h5_currentgrp = "/"//trim(h5_mode_groupname)//"/T_tot_ntv_i"
    ...
    call h5_append_double_0(dsetid_ntv, T_tot_ntv_i(time_index), time_index)
