...
subroutine GreenMom(mpar,lperp,x1,x2,x3,x4,Imnl)
  ...
  implicit none
  double precision, parameter :: addimpart=0.1d0
...
!
  ...
  theta=dcmplx(1.d0,sign(addimpart,dimag(oldarg)))
!
  do m=0,mpar
    do n=0,mpar
...
!
    ...
    D_21(i) = D_21(i)-D_12a
!
!    write(3001,*) rb(i),real(Imnkl(0,0,0,0)),dimag(Imnkl(0,0,0,0))
  enddo
!
...
!  np=npoi
!  do i=1,np
!    rloc=(rb(1)*dfloat(np-i)+rb(npoib)*dfloat(i-1))/dfloat(np-1)
!    call get_matrix
!    write(3001,*) xarr(i),real(amat_arr(1,1,i)),dimag(amat_arr(1,1,i))
!  enddo
!
...
!
  write(3001,*) r,x1,x2,x3,x4,real(Imnkl(0,0,0,0)),dimag(Imnkl(0,0,0,0))
!
