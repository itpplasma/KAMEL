...
subroutine rhs_balance_orig(x, y, dy)
    ...
    if (isw_rhs .eq. 0) then
        ...
        call rhs_balance_source_orig(x, y, dy)
        rhsvec = dy
    end if

end subroutine rhs_balance_orig


subroutine initialize_rhs_orig(y, dy)
    ...
    call rhs_balance_orig(x, y, dy)

    isw_rhs = 1
    if (allocated(amat)) deallocate (irow, icol, amat, rhsvec)
    allocate (irow(nz), icol(nz), amat(nz), rhsvec(nsize))

end subroutine initialize_rhs_orig


subroutine rhs_balance_source_orig(x, y, dy)
...
end subroutine rhs_balance_source_orig
