program test_periodic_kim_selector
    ...
    if (trim(kim_ion_transport_model) /= 'integral') &
        error stop 'integral ion transport is not the production default'
    if (ion_transport_model_id('integral') /= ION_TRANSPORT_INTEGRAL) &
        error stop 'integral ion transport selector is broken'
