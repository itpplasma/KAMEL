...
class KimGUI:
    ...
    def create_configuration_tab(self):
        ...
        default_button = tk.Button(frame, text="Default", command=self.load_default_config, bg='#4CAF50', fg='#000000')
        default_button.grid(row=1, column=0, pady=5)

        load_config_button = tk.Button(frame, text="Load Config", command=self.load_config, bg='#4CAF50', fg='#000000')
        load_config_button.grid(row=1, column=1, pady=5)

        save_config_button = tk.Button(frame, text="Save Config", command=self.save_config, bg='#4CAF50', fg='#000000')
        save_config_button.grid(row=1, column=2, pady=5)

        # Textboxes for displaying and manipulating the loaded name list
        self.name_list_text = scrolledtext.ScrolledText(frame, wrap=tk.WORD, width=70, height=30, bg='#000000', fg='#FFFFFF')
        self.name_list_text.grid(row=2, column=0, columnspan=3, padx=10, pady=(0, 10))

        # Textbox for specifying the save location
        save_location_label = tk.Label(frame, text="Save Location:", foreground='#FFFFFF', background='#303030')
        ...
        save_location_button = tk.Button(frame, text="Browse", command=self.browse_save_location, bg='#4CAF50', fg='#000000')
        save_location_button.grid(row=3, column=2, pady=(10, 0), padx=5, sticky='w')


    def browse_save_location(self):
        # Open a file dialog to select a save location
        save_location = filedialog.asksaveasfilename(defaultextension=".txt", filetypes=[("Text files", "*.nml"), ("All files", "*.*")])
    ...
    def load_config(self):
        ...
        file_path = filedialog.askopenfilename(filetypes=[("Text files", "*.nml"), ("All files", "*.*")])
    ...
    def save_config(self):
        # Open a file dialog to select a save location
        save_path = filedialog.asksaveasfilename(defaultextension=".txt", filetypes=[("Text files", "*.nml"), ("All files", "*.*")])
    ...
    def create_run_tab(self):
        ...
        run_button = tk.Button(self.run_tab, text="Run Executable", command=self.start_executable, bg='#4CAF50', fg='#FFFFFF')  # Button colors
        run_button.pack(pady=10)

        # Text box to display command line output
        self.output_text = scrolledtext.ScrolledText(self.run_tab, wrap=tk.WORD, width=60, height=10, bg='#303030', fg='#FFFFFF')  # Text box colors
    ...
    def create_output_tab(self):
        ...
        plot_button = tk.Button(frame, text="Plot Data", command=self.plot_data, bg='#4CAF50', fg='#000000')
        plot_button.grid(row=1, column=0, pady=10)

        # Text box to display command line output
        self.output_text = scrolledtext.ScrolledText(frame, wrap=tk.WORD, width=60, height=10, bg='#303030', fg='#000000')
