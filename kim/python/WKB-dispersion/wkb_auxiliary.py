...
def determine_phase_of_phi_WKB_struct(dat:dict):
    ...
    for i, r in enumerate(dat['r_found_only_one']):
        if r > dat['r_res']:
            ...
            #integral[i] = np.trapz(dat['k_r1_only_one'][intrange:i], dat['r_found_only_one'][intrange:i])
        if r < dat['r_res']:
            integral[i] = -np.trapz(dat['k_r1_only_one'][i:intrange], dat['r_found_only_one'][i:intrange])
    ...
    for i, r in enumerate(dat['r_found']):
        if not r == dat['r_found'][i-1]:
            ...
            dat['k_r1_only_one'].append(\
                ...
                -1j*np.min(np.abs(np.imag(dat['k_r1'][np.where(dat['r_found'] == dat['r_found'][i])])))
            ...
            dat['k_r2_only_one'].append(\
                ...
                +1j*np.min(np.abs(np.imag(dat['k_r2'][np.where(dat['r_found'] == dat['r_found'][i])])))
                )
