...
% *) write(obj)
% *) run(obj)
% *) run_balance_init(obj, m, n, cfile, gfile, fluxdatapath, gpecpath, neprof, Teprof, Tiprof, vtprof, dapath)
% *) loadOutput(obj)
% *) plotDe22(obj)
...
%author:   Philipp Ulbl
%created:  14.01.2020
%edited: Markus Markl xx.02.2021
    ...
    properties(SetAccess = private)
		...
		optionsnml		%options for balance code (represents balance_conf.nml)
        factors = 0     %factors for V-shift
		% added by Markus Markl, 05.03.2021
		fac_n
		fac_Ti
		fac_Te
		fac_vz
    ...
    methods
        function obj = Balance(runpath, shot, time, name, hdf5file)
            %##############################################################
            %function obj = Balance(runpath, shot, time, name, hdf5file)
            %##############################################################
            % description:
			...
			% hdf5file  ... path/to/hdf/file.hdf5 ; input hdf5 file, data
            % from pre run
            %##############################################################
            ...
            % set hdf5 file, this is the "input" or prerun hdf5 file
        ...
        function setModes(obj, m, n)
            ...
            % n     ... mode number n
            %##############################################################
            % Edited: 15.02.2021, Markus Markl
            % saves mode numbers to hdf5 file
            %##############################################################
            ...
            obj.m = m;
            obj.n = n;
            % save the mode numbers to hdf5 file

            %obj.writeHDF5(obj.hdf5file, '/input/', 'm', 'poloidal mode number', '1');
            %obj.writeHDF5(obj.hdf5file, '/input/', 'n', 'toroidal mode number', '1');
        ...
        function setCoil(obj, cfile, pfile)
            ...
            % write coil file data to hdf5 file
            cfiledata = importdata(cfile);
            try
                % try creating dataset, if it already exists, overwrite it
                h5create(obj.hdf5file, '/input/cfile', size(cfiledata));
            catch
                disp('cfile already exists, will overwrite')
            end
            
            % write data to dataset
            h5write(obj.hdf5file, '/input/cfile', cfiledata);
            % write attribute to data, i.e. description of the data
            % contained
            h5writeatt(obj.hdf5file, '/input/cfile', 'description', 'Data of coil file');
            %#########################################################
        ...
        function setEqui(obj, gfile, fluxdatapath, calc_amn)
            ...
            %##############################################################
            % Edited: 15.02.2021, Markus Markl
            % now saves Btor, r_big, r_sep_real to hdf5 file
            %##############################################################
            ...
            obj.writeHDF5(obj.hdf5file, '/input/', 'b_tor', 'toroidal magnetic field at center', 'G');
            obj.writeHDF5(obj.hdf5file, '/input/', 'r_big', 'big torus radius', 'cm');
            obj.writeHDF5(obj.hdf5file, '/input/', 'r_sep_real', 'location of separatrix in equilibrium', 'cm');
        ...
        function setTMHDCode(obj, ttype, tpath)
            ...
            %##############################################################
            % Edited: 15.02.2021, by Markus Markl
            % Added a save procedure that writes the resonant current
            % values to hdf5 file.
            %##############################################################
            ...
            if(strcmp(ttype, 'GPEC'))
                ...
                % save the relevant data to hdf5 file
                obj.writeHDF5(obj.hdf5file, '/GPEC/', 'I_tmhd', 'resonant currents from toroidal mhd code', 'statA (c=1)');
        ...
        function setProfiles(obj, neprof, Teprof, Tiprof, vtprof, copy)
            ...
            obj.profiles.export2HDF5(obj.hdf5file, '/preprocprof/')
        end

        function setKiLCA(obj, ion_mass)
            %##############################################################
            ...
            %##############################################################
            % Edited: 15.02.2021, Markus Markl
            % Save the current and form factors now to hdf5 file
            %##############################################################
            ...
            % save the relevant data to hdf6 file
            ...
            obj.writeHDF5(obj.hdf5file, '/output/', 'q_res', 'resonant safety factor', '1');
            obj.writeHDF5(obj.hdf5file, '/output/', 'legacy_specfactors', 'spectral factors Wmn', '1');
            obj.writeHDF5(obj.hdf5file, '/output/', 'legacy_I_rescaled', 'resonant currents rescaled with Wmn', 'statA (c=1)');

            % write to hdf5 file
            obj.writeHDF5(obj.hdf5file, '/output/', 'scalefactors_sq', 'scale factors Cmn^2', '1');
			obj.writeHDF5(obj.hdf5file, '/output/', 'r_res', 'resonant surface location', 'cm');
            obj.writeHDF5(obj.hdf5file, '/output/', 'd_res', 'resonant layer thickness', 'cm');
            obj.writeHDF5(obj.hdf5file, '/output/', 'I_res', 'resonant currents', 'statA (c=1)');
            obj.writeHDF5(obj.hdf5file, '/output/', 'I_tmhd', 'resonant currents from toroidal mhd code', 'statA (c=1)');

            obj.writeHDF5(obj.hdf5file, '/output/', 'dis_zero_veperp', 'absolute distance for each mode from veperp zero', 'cm');
            obj.writeHDF5(obj.hdf5file, '/output/', 'dis_zero_vExB', 'absolute distance for each mode from vExB zero', 'cm');
            obj.writeHDF5(obj.hdf5file, '/output/', 'zero_veperp', 'zeros of veperp', 'cm');
            obj.writeHDF5(obj.hdf5file, '/output/', 'zero_vExB', 'zeros of vExB', 'cm');
			obj.writeHDF5(obj.hdf5file, '/input/', 'm_ion', 'ion mass number', '1');



            %export KiLCA data if it is there (only with pre-computation)
            % this is the export method of the KiLCA interface
            % it creates also a "file" structure in the hdf5 file
            try
                obj.kil_flre.export2HDF5(obj.hdf5file, '/KiLCA_flre/');
                obj.kil_vacuum.export2HDF5(obj.hdf5file, '/KiLCA_vac/');
            catch
            end
        end
        ...
        function setDaEstimation(obj, path_da)
            ...
            obj.writeHDF5(obj.hdf5file, '/output/', 'Da_res', 'resonant De22', 'cm^2  s^{-1}');

            %write 2 file for balance
            obj.da_est.write([obj.path_profiles, 'Da.dat']);
            % write to hdf5
            obj.da_est.export2HDF5(obj.hdf5file, '/da_estimation/');
        end
		...
		function setOptionsNML(obj, opt)
		    %##############################################################
            %function setOptionsNML(opt)
            %##############################################################
            % description:
            %--------------------------------------------------------------
            % set options for the balance code in the form of a namelist
			% file.
            %##############################################################
            % input:
            %--------------------------------------------------------------
            % opt   ... InputFile namelist object, if empty it takes the
			% file from the blueprints folder
			%##############################################################
			% Edited: 24.02.2021 by Markus Markl
            %##############################################################

			if(nargin < 2 || isempty(opt))
				obj.optionsnml = InputFile([obj.LIB_BALANCE,'blueprints/balance_conf.nml']);
				obj.optionsnml.read();
				obj.optionsnml.BALANCENML.btor = obj.b_tor;
				obj.optionsnml.BALANCENML.rtor = obj.r_big;
				obj.optionsnml.BALANCENML.rmin = obj.r_sta;
				obj.optionsnml.BALANCENML.rmax = obj.r_ant;
				obj.optionsnml.BALANCENML.rb_cut_out = obj.rb_cut_out;
				obj.optionsnml.BALANCENML.re_cut_out = obj.re_cut_out;

			else
				obj.optionsnml = opt;
				%obj.optionsnml.read();
			end
			obj.r_sep_real = h5read(obj.hdf5file, '/input/r_sep_real');
			...
			if (not(obj.optionsnml.BALANCENML.paramscan))
				obj.setFactors([1], 'fac_n')
				obj.setFactors([1], 'fac_Ti')
				obj.setFactors([1], 'fac_Te')
				obj.setFactors([1], 'fac_vz')
			end

		end

	% edited: Markus Markl, 03.03. 2021
        function setFactors(obj, f, type)
            %##############################################################
            %function setFactors(obj, f)
            %##############################################################
            % description:
            %--------------------------------------------------------------
            % set factors for parameter scans. They are saved into the hdf5
			% file. The actual scan is done in the fortran code.
            %##############################################################
            % input:
            %--------------------------------------------------------------
            % f     ... vector of factors
			% type  ... indicates the profile that is to be changed (i.e.
			%			n, Ti, Te, vz)
            %##############################################################

            %obj.factors = f;

			if (strcmp(type,'fac_n'))
				obj.fac_n = f;
				obj.writeHDF5(obj.h5out, '/factors/', 'fac_n', ...
					'factors for n', '1');

			elseif(strcmp(type, 'fac_Te'))
				obj.fac_Te = f;
				obj.writeHDF5(obj.h5out, '/factors/', 'fac_Te', ...
					'factors for Te', '1');

			elseif(strcmp(type, 'fac_Ti'))
				obj.fac_Ti = f;
				obj.writeHDF5(obj.h5out, '/factors/', 'fac_Ti', ...
					'factors for Ti', '1');

			elseif(strcmp(type, 'fac_vz'))
				obj.fac_vz = f;
				obj.writeHDF5(obj.h5out, '/factors/', 'fac_vz', ...
					'factors for vz', '1');
				if (any(f~=1.0))
					if(~isempty(obj.profiles))
						obj.B0_ervzfac = obj.profiles.B_out;
						obj.ErVzfac = obj.profiles.r_out .* obj.B0_ervzfac ./ ...
							(obj.profiles.r_big * obj.profiles.qp.y_out .* 3e10);
						obj.writeHDF5(obj.h5out, '/factors/', 'ErVzfac', ...
							'factor to rescale Er profile if velocity scan is done','1');
						...
						obj.ErVzfac = prof_r_out .* obj.B0_ervzfac ./ ...
							(prof_rbig * prof_q .* 3e10);
						obj.writeHDF5(obj.h5out, '/factors/', 'ErVzfac', ...
							'factor to rescale Er profile if velocity scan is done', '1');
						disp('Written ErVzfac')
        ...
        function write(obj)
            ...
            %[~, ~] = system(['ln -fsv ', obj.LIB_BALANCE, 'template_experimental/* ', obj.path_run])
        ...
        function load_prerun(obj, ion_mass)
           %###############################################################
           % function load_prerun(obj)
           %###############################################################
           % description:
           %---------------------------------------------------------------
           % load the prerun hdf5 file, since prerun and actual run can 
           ...
           %     obj.(runname).PROF_PATH = obj.path_profiles;
				...
				obj.(runname).background.mi = obj.m_ion;
    		...
    		%	if(~exist(obj.(runname).pathofrun, 'dir'))
            %        obj.(runname).write();
			%	end
        ...
        function run(obj)
            ...
            for i = 1:numel(obj.m)
                ...
                %change to run path and change the mode numbers in mode.in
                cd(obj.path_run)
				system('rm modes.in');
                ...
                % Note M.: write the output files directly in hdf5 file
                %outfiles = balanceoutput.OUTFILES; % get the names of the output files from the
                % balanceoutput class
        ...
        function loadOutput(obj)
            ...
            for i = 1:numel(obj.m)
                ...
                obj.outputs{i}.loadOutput(mnoutpath); % loads f1000 and f5000 files into the object
            end
            ...
            for j = 1:numel(obj.m)
                if(~obj.optionsnml.BALANCENML.flag_run_time_evolution) % if NO time evolution
                %if(~obj.options.flag_run_time_evolution) % if NO time evolution
        ...
        function export2HDF5(obj, path, name)
            ...
            if(obj.optionsnml.BALANCENML.flag_run_time_evolution)
    ...
    methods(Access = private)
        ...
        function reset_profiles(obj)
            %##############################################################
            % function reset_profiles(obj)
            %##############################################################
            % description:
            %--------------------------------------------------------------
            % save the pre processed profiles to the /profiles/ directory
            % in the format of *.dat files. They are needed for the 
            % calculations done by KiLCA. 
            % This should be done in a time evolution run, after every
            % loop of a mode number.
            %##############################################################
