...
%created:  14.01.2020
    ...
    methods
        ...
        function setCoil_fromhdf5(obj, hdf5file, pfile)
            %##############################################################
            %function setCoil_fromhdf5(obj, hdf5file, pfile)
            %##############################################################
            % description:
            %--------------------------------------------------------------
            % read coil data from hdf5 file for balance run
            %##############################################################
            % input:
            %--------------------------------------------------------------
            % hdf5file ... location of the hdf5file where coil data is
            % safed.
            % pfile   ... (optional) location of Kisslinger coil file
            %             (forces calculation if not present)
            % ----
            % to do: write routine to calculate via pfile and safe to hdf5
            % file
            %##############################################################

            try
                obj.file_coil_raw = h5read(hdf5file,'/input/cfile');
            catch
                error('No coil data in hdf5 file')
            end
        ...
        function setEqui_fromhdf5(obj, hdf5file, fluxdatapath, calc_amn)
            %##############################################################
            %function setEqui(obj, gfile, fluxdatapath, use_amn)
            %##############################################################
            % description:
            ...
            % try if gfile data is there, if not error
            try
                obj.equi_data = h5read(hdf5file,'/input/gfile');
            catch
                error('No gfile data in hdf5 file')
            end
            ...
            if(run_fourier==true)
                ...
                %path to exec
                fourierpath = [obj.LIB_BALANCE, 'fourier/'];
                % TO DO: CHANGE EXECUTABLE SUCH THAT THE OUTPUT IS WRITTEN
                % IN HDF5 FILE!!
        ...
        function write(obj)
            ...
            [~, ~] = system(['ln -fsv ', '/temp/markl_m/libneo/matlab/BALANCE/template_experimental/* ', obj.path_run]);
        ...
        function run(obj)
            ...
            for i = 1:numel(obj.m)
                ...
                %dir for output
                % Note M.: create here the hdf5 file?
...
%#########################################################################
% ---- added by Markus Markl on 09.02.21 ----
        function run_balance_init(obj, m, n, cfile, gfile, fluxdatapath, gpecpath, neprof, Teprof, Tiprof, vtprof, dapath)
          %##############################################################
          %function run_balance_init(obj)
          %##############################################################
          % description:
          %--------------------------------------------------------------
          % Runs the first part of the balance cycle. It creates the
          % necessary files and directory structure for the balance run.
          %
          % Author: Markus Markl
          % Created: 08.02.2021
          %##############################################################
          ...
          %--------------------------------------------------------------
          % m     ... mode number m
          % n     ... mode number n, must be same dimension as n
          % cfile ... location of raw coil file
          % gfile ... location of the g file
          % fluxdatapath ... directory of the fluxdata
          % gpecpath ... path to output of tmhd Code
          % neprof ... location of ne profile from experiment
          % Teprof  ... location of Te profile from experiment
          % Tiprof  ... location of Ti profile from experiment
          % vtprof  ... location of vt profile from experiment
          % dapath ... location of files used for Da estimation
          % opt   ... balanceoptions object, if empty, defailt options are used
          %##############################################################
          disp('+++++++ Balance Configuration +++++++')
          disp(['shot ', num2str(obj.shot), ' at ', num2str(obj.time), 'ms'])
          disp('-- set modes --')
          obj.setModes(m, n);
          disp('-- set coil --')
          obj.setCoil(cfile);
          disp('-- set equilibrium --')
          obj.setEqui(gfile, fluxdatapath);
          disp('-- set tMHD code --')
          obj.setTMHDCode('GPEC',gpecpath);
          disp('-- set profiles --')
          obj.setProfiles(neprof, Teprof, Tiprof, vtprof);
          disp('-- set kilca --')
          obj.setKiLCA();
          disp('-- set da estimation --')
          obj.setDaEstimation(dapath);
          disp('-- set options --')
          obj.setOptions();
          disp('-- write --')
          obj.write();
          disp('-- balance configuration finished --')
        end
%#########################################################################
...
%             obj.kil_flre.loadOutput();
        ...
        function export2HDF5_byfname(obj, fullpathfname)
            %##############################################################
            %function export2HDF5(obj, path, name)
            %##############################################################
            % description:
            %--------------------------------------------------------------
            % exports most relevant data to existing hdf5 file.
            % a suitable hdf5 file has to be present
            %##############################################################
            % input:
            %--------------------------------------------------------------
            % fullpathfname
