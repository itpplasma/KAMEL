...
%created:  08.01.2020
    ...
    methods
        ...
        function export2HDF5(obj, fname, loc)
            ...
            %##############################################################
            % edited: 16.02.2021, Markus Markl
            % added the export of the n, T, v, Er and q profiles
            %##############################################################

            obj.writeHDF5(fname, [loc,'/equil/'], 'r', 'equivalent radius from equilibrium', 'cm');
            obj.writeHDF5(fname, [loc,'/equil/'], 'q', 'safety factor from equilibrium', '1');

            obj.writeHDF5(fname, [loc,'/equil/'], 'psi_pol_norm', 'normalized poloidal flux', '1');
            obj.writeHDF5(fname, loc, 'r_out', 'output radius vector', 'cm');
            obj.Te.writeHDF5(fname, loc, 'y_out', 'electron temperature profile', 'eV');
            obj.Ti.writeHDF5(fname, loc, 'y_out', 'ion temperature profile', 'eV');
            obj.ne.writeHDF5(fname, loc, 'y_out', 'density profile', 'cm^{-3}');
            obj.vt.writeHDF5(fname, loc, 'y_out', 'toroidal velocity profile', 'cm s^{-1}');
            obj.vth.writeHDF5(fname, loc, 'y_out', 'poloidal velocity profile', 'cm s^{-1}');
            obj.qp.writeHDF5(fname, loc, 'y_out', 'safety factor profile', '1');
            obj.Er.writeHDF5(fname, loc, 'y_out', 'radial electric field profile', 'statV cm^{-1}');
