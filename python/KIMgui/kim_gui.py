...
class KIMConfigGUI:
    ...
    def create_config_widgets(self):
        ...
        ttk.Entry(path_entry_frame, textvariable=self.runpath, width=50).pack(side='left', fill='x', expand=True)
        ttk.Button(path_entry_frame, text="Browse", command=self.browse_runpath).pack(side='right', padx=(5,0))
        ...
        ttk.Button(button_frame, text="Reset to Default", command=self.load_default_config).pack(side='left', padx=5)
    ...
    def create_config_fields(self, parent, fields, section_name):
        for field_name, label, field_type, default_value in fields:
            ...
            if field_type == bool:
                ...
                combo = ttk.Combobox(frame, textvariable=var, values=['Krook', 'FokkerPlanck'], width=12)
                combo.pack(side='left')
            elif field_name == 'type_of_run':
                var = tk.StringVar(value=default_value)
                combo = ttk.Combobox(frame, textvariable=var, values=['electrostatic', 'electromagnetic'], width=12)
                combo.pack(side='left')
            else:
                var = tk.StringVar(value=str(default_value))
                ttk.Entry(frame, textvariable=var, width=15).pack(side='left')
    ...
    def create_run_widgets(self):
        ...
        self.gen_profiles_btn = ttk.Button(profile_frame, text="Generate Constant Profiles", 
                                          command=self.generate_profiles)
        ...
        self.use_config_cb = ttk.Checkbutton(run_frame, text="Use GUI Configuration", 
                                            variable=self.use_gui_config)
        ...
        ttk.Button(results_frame, text="Open Output Directory", command=self.open_output_dir).pack(side='left', padx=10)
        ...
        try:
                ...
                if code_path:
                    ...
                    if os.path.exists(source_kim_path):
                        try:
                            ...
                            self.output_text.insert(tk.END, f"Created softlink to KIM.x from {source_kim_path}\n")
                            self.output_text.see(tk.END)
                        except OSError as e:
                            raise Exception(f"Failed to create softlink to KIM.x: {str(e)}")
                    else:
                        raise Exception(f"KIM.x not found at {source_kim_path}. Please build KIM first with 'make KIM'.")
                else:
                    raise Exception("CODE environment variable not set. Please set CODE to point to your KAMEL installation.")
            ...
            if self.use_gui_config.get():
                ...
                try:
                    ...
                    self.output_text.insert(tk.END, "Updated KIM_config.nml with GUI configuration\n")
                    self.output_text.see(tk.END)
                except Exception as e:
                ...
                if not os.path.exists(config_path):
                    raise Exception(f"No existing KIM_config.nml found at {config_path}. Please create one first or enable 'Use GUI Configuration'.")
    ...
    def plot_results(self):
        try:
            ...
            try:
                # Load phi_sol
                phi_sol = np.loadtxt(os.path.join(runpath, f'out/m{m_mode}_n{n_mode}/fields/phi_sol.dat'))
                r = phi_sol[:,0]
                phi = phi_sol[:,1]
                ...
                E_perp = np.loadtxt(os.path.join(runpath, f'out/m{m_mode}_n{n_mode}/fields/E_perp.dat'))
                E_perp_psi = np.loadtxt(os.path.join(runpath, f'out/m{m_mode}_n{n_mode}/fields/E_perp_psi.dat'))
                E_perp_MA = np.loadtxt(os.path.join(runpath, f'out/m{m_mode}_n{n_mode}/fields/E_perp_MA.dat'))
                
                # Plot combined E_perp components (absolute values only)
                axes[1].plot(E_perp[:,0], np.abs(E_perp[:,1]), label=r'$|E_\perp|$', linewidth=2)
                axes[1].plot(E_perp_psi[:,0], np.abs(E_perp_psi[:,1]), label=r'$|E_\perp^{[\psi]}|$', linewidth=2)
                axes[1].plot(E_perp_MA[:,0], np.abs(E_perp_MA[:,1]), label=r'$|E_\perp^{MA}|$', linewidth=2)
                axes[1].set_xlabel('r [cm]')
                axes[1].set_ylabel('Electric Field')
                axes[1].legend()
                
                # Load Br_pert
                Br_pert = np.loadtxt(os.path.join(runpath, f'out/m{m_mode}_n{n_mode}/fields/Br_pert.dat'))
                ...
                axes[0].text(0.5, 0.5, f'Results not found:\n{str(e)}', 
