...
def calc_needed_bessel_of_mphi(mphi, eval_b,mode='Integral'):
    if mode=='Integral':
        ...
        if np.isscalar(eval_b):
            BesselProd0, _ = spint.quad(lambda theta: bessel_integrand(theta, eval_b, mphi), 0, np.pi,complex_func=True)
            BesselProd1, _ = spint.quad(lambda theta: bessel_integrand(theta, eval_b, mphi-1), 0, np.pi,complex_func=True)
        #If integration method romb is used in cxroots eval_b may be vector -> this needs to be handled
        else:
            intfun=lambda eval_b: spint.quad(lambda theta: bessel_integrand(theta, eval_b, mphi), 0, np.pi,complex_func=True)[0]
            vec_int=np.vectorize(intfun)
            BesselProd0=vec_int(eval_b)
            intfun=lambda eval_b: spint.quad(lambda theta: bessel_integrand(theta, eval_b, mphi-1), 0, np.pi,complex_func=True)[0]
            vec_int=np.vectorize(intfun)
...
def BesselParabolicExpansion(z):
    f=lambda z,n: 1/np.sqrt(2*np.pi*z)*(1+(4*n**2-1)/(8*z)+((4*n**2-1)*(4*n**2-9))/(factorial(2)*(8*z)**2)-((4*n**2-1)*(4*n**2-9)*(4*n**2-25)/(factorial(3)*(8*z)**3)))
    B0=f(z,0)
    B1=f(z,-1)
    return B0,B1
