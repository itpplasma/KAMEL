...
class KIMDispersion_FokkerPlanck(KIMDispersionEquation):
    ...
    def dispersion_equation(self, kr, r_indx):
        ...
        dispersion_equation = (
            self.general_dat["kperp"][r_indx] ** 2 + self.general_dat["kp"][r_indx] ** 2 + 0j
        )
        ...
        for spec in self.species:
            ...
            for m_phi in range(0, self.options["max_cyclotron_harmonic"] + 1):
                ...
                if np.isnan(BesselProd0) or np.isnan(BesselProd1):
                    print(
                        f"FokkerPlanck: BesselProd0 or BesselProd1 contains NaNs, eval_b = {eval_b}, m_phi = {m_phi}"
                    )
                    print(f"BesselProd0: {BesselProd0}, BesselProd1: {BesselProd1}")

                dispersion_equation_temp = (
                    (
                        -1.0 / self.spec_dat[spec]["lambda_D"][r_indx] ** 2
                        + 1j
                        * self.spec_dat[spec]["vT"][r_indx] ** 2.0
                        * self.general_dat["ks"][r_indx]
                    )
                    / (
                        self.spec_dat[spec]["lambda_D"][r_indx] ** 2
                        * self.spec_dat[spec]["omega_c"][r_indx]
                        * self.spec_dat[spec]["nu"][r_indx]
                    )
                    * (
