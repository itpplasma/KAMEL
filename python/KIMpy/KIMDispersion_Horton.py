...
class KIMDispersion_Horton(KIMDispersionEquation):
    ...
    def dispersion_equation(self, kr: complex, r_indx: int) -> complex:
        ...
        for spec in self.species:
            self.spec_dat[spec]["om_n"] = (
                ky
                * sol
                / (
                    self.spec_dat[spec]["charge"]
                    * self.equil_dat["B0"][r_indx]
                    * self.spec_dat[spec]["n"][r_indx]
                )
                * self.spec_dat[spec]["T"][r_indx]
                * ev
                * self.spec_dat[spec]["dndr"][r_indx]
            )
            self.spec_dat[spec]["om_T"] = (
                ky
                * sol
                / (self.spec_dat[spec]["charge"] * self.equil_dat["B0"][r_indx])
                * self.spec_dat[spec]["dTdr"][r_indx]
                * ev
            )

            self.spec_dat[spec]["z"] = om_prime / (
                np.sqrt(2) * self.general_dat["kp"][r_indx] * self.spec_dat[spec]["vT"][r_indx]
            )
            self.spec_dat[spec]["W"] = (
                self.spec_dat[spec]["om_n"]
                - om_prime
                + self.spec_dat[spec]["om_T"] * (self.spec_dat[spec]["z"] ** 2 + 0.5)
            ) * self.spec_dat[spec]["z"] / om_prime * plasma_disp(
                self.spec_dat[spec]["z"]
            ) + self.spec_dat[
                spec
            ][
                "om_T"
            ...
            self.spec_dat[spec]["nom"] = self.spec_dat[spec]["lambda_D"][r_indx] ** -2 * (
                self.spec_dat[spec]["W"]
                - 1.0
                - self.spec_dat[spec]["om_T"]
                * self.spec_dat[spec]["z"]
                / om_prime
                * plasma_disp(self.spec_dat[spec]["z"])
            )
            self.spec_dat[spec]["denom"] = (
                self.spec_dat[spec]["W"]
                * self.spec_dat[spec]["vT"][r_indx] ** 2
                / (
                    self.spec_dat[spec]["omega_c"][r_indx] ** 2
