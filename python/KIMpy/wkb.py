...
class KIM_WKB:
    ...
    def calc_dispersion_relation_k_of_r(self, mode, collisions):
        ...
        r_used = []
        res_sorted = []
        contour = cx.Rectangle(
            x_range=[-self.contour_limit, self.contour_limit],
            y_range=[-self.contour_limit, self.contour_limit],
        )
    ...
    def find_res_surface(self):
        ...
        self.res_surf_val = np.interp(
            self.m_mode / self.n_mode, np.abs(self.general_dat["q"]), self.general_dat["r"]
        )
    ...
    def generate_non_uniform_grid(self):
        ...
        self.r_new = np.array(
            gobj.gen_grid(self.general_dat["r"], gobj.r_res, 5.0, self.options["n_points"])
        )
        ...
        for key in self.general_dat:
            try:
                if len(self.general_dat[key]) == self.general_dat["prof_length"]:
                    if key == "old_r":
                        continue
                    self.general_dat[key] = np.interp(
                        self.r_new, self.general_dat["old_r"], self.general_dat[key]
                    )
            except:
                pass
        for spec in self.species:
            for key in self.spec_dat[spec]:
                if key == "n" or key == "T":
                    self.spec_dat[spec][key] = np.interp(
                        self.general_dat["r"], self.general_dat["old_r"], self.spec_dat[spec][key]
                    )
            ...
            if self.scale_coll_frequency:
                for spec in self.species:
                    self.spec_dat[spec]["nu"] = (
                        self.spec_dat[spec]["nu"] * self.coll_frequency_scale_factor
                    )
        self.calc_all_derivs()

        self.general_dat["ks"] = (
            self.m_mode * self.equil_dat["hz"] - self.n_mode * self.equil_dat["hth"] / self.R0
        ) / self.general_dat["r"]
        self.general_dat["kp"] = np.abs(
            self.m_mode / self.general_dat["r"] * self.equil_dat["hth"]
            + self.n_mode / self.R0 * self.equil_dat["hz"]
        )
        self.general_dat["om_E"] = (
            -sol * self.general_dat["ks"] * self.general_dat["Er"] / self.equil_dat["B0"]
        )

        for spec in self.species:
            self.spec_dat[spec]["vT"] = np.sqrt(
                self.spec_dat[spec]["T"] * ev / self.spec_dat[spec]["mass"]
            )
            self.spec_dat[spec]["omega_c"] = (
                np.abs(self.spec_dat[spec]["charge"])
                * self.equil_dat["B0"]
                / (self.spec_dat[spec]["mass"] * sol)
            )
            self.spec_dat[spec]["lambda_D"] = np.sqrt(
                self.spec_dat[spec]["T"]
                * ev
                / (4.0 * np.pi * self.spec_dat[spec]["n"] * self.spec_dat[spec]["charge"] ** 2)
            )
            self.spec_dat[spec]["A1"] = (
                self.spec_dat[spec]["dndr"] / self.spec_dat[spec]["n"]
                - self.spec_dat[spec]["charge"]
                / (self.spec_dat[spec]["T"] * ev)
                * self.general_dat["Er"]
                - 3 / (2 * self.spec_dat[spec]["T"]) * self.spec_dat[spec]["dTdr"]
            )
            self.spec_dat[spec]["A2"] = self.spec_dat[spec]["dTdr"] / self.spec_dat[spec]["T"]
            self.spec_dat[spec]["z0"] = -(
                self.general_dat["om_E"] - self.options["omega"] - 1j * self.spec_dat[spec]["nu"]
            ) / (np.abs(self.general_dat["kp"]) * np.sqrt(2) * self.spec_dat[spec]["vT"])
            self.spec_dat[spec]["rho_TL"] = self.spec_dat[spec]["vT"] / np.abs(
                self.spec_dat[spec]["omega_c"]
            )
    ...
    def calc_equilibrium(self):
        ...
        f = (
            ...
            * u
            / (
                self.R0**2 * self.general_dat["q"][self.val2ind(r, self.general_dat["r"])] ** 2
                + r**2
            )
            - 8 * pi * dpress_prof[self.val2ind(r, self.general_dat["r"])]
        )
        u0 = self.btor**2 * (
            1 + self.general_dat["r"][0] ** 2 / (self.R0**2 * self.general_dat["q"][0] ** 2)
        )
        u = solve_ivp(
            f,
            t_span=[self.general_dat["r"][0], self.general_dat["r"][-1]],
            y0=[u0],
            t_eval=self.general_dat["r"],
        )
        u = u.y[0]
        self.equil_dat["B0z"] = np.sign(self.btor) * np.sqrt(
            u / (1 + self.general_dat["r"] ** 2 / (self.R0**2 * self.general_dat["q"] ** 2))
        )
        self.equil_dat["B0th"] = (
            self.equil_dat["B0z"] * self.general_dat["r"] / (self.general_dat["q"] * self.R0)
        )
    ...
    def calc_collision_frequency(self):
        Lee = (
            23.5
            - np.log(np.sqrt(self.spec_dat["e"]["n"]) * self.spec_dat["e"]["T"] ** (-5 / 4))
            - np.sqrt(1e-5 + ((np.log(self.spec_dat["e"]["T"]) - 2) ** 2) / 16)
        )
        self.spec_dat["e"]["nu"] = (
            5.8e-6 * self.spec_dat["e"]["n"] * Lee * self.spec_dat["e"]["T"] ** (-3 / 2)
        )  # Collision frequency
        ...
        for test in self.ion_species:
            ...
            self.spec_dat["e"]["nu"] = (
                self.spec_dat["e"]["nu"]
                + 7.7e-6
                * self.spec_dat["e"]["T"] ** -1.5
                * self.spec_dat[test]["n"]
                * Lei
                * self.spec_dat[test]["Zi"] ** 2
            )
            # nui ie: collision frequency ions with electrons
            self.spec_dat[test]["nu"] = (
                1.8e-7
                * self.spec_dat[test]["Ai"] ** (-1 / 2)
                * self.spec_dat[test]["T"] ** (-3 / 2)
                * self.spec_dat["e"]["n"]
                * self.spec_dat[test]["Zi"] ** 2
                * Lei
            )
            for field in self.ion_species:
                Liiprime = 23 - np.log(
                    self.spec_dat[test]["Zi"]
                    * self.spec_dat[field]["Zi"]
                    * (self.spec_dat[test]["Ai"] + self.spec_dat[field]["Ai"])
                    / (
                        self.spec_dat[test]["T"] * self.spec_dat[field]["Ai"]
                        + self.spec_dat[field]["T"] * self.spec_dat[test]["Ai"]
                    )
                    * np.sqrt(
                        self.spec_dat[test]["n"]
                        * self.spec_dat[test]["Zi"] ** 2
                        / self.spec_dat[test]["T"]
                        + self.spec_dat[field]["n"]
                        * self.spec_dat[field]["Zi"] ** 2
                        / self.spec_dat[field]["T"]
                    )
                )
                ...
                self.spec_dat[test]["nu"] = (
                    self.spec_dat[test]["nu"]
                    + 1.8e-7
                    * self.spec_dat[test]["Ai"] ** -0.5
                    * self.spec_dat[test]["T"] ** -1.5
                    * self.spec_dat[field]["n"]
                    * self.spec_dat[test]["Zi"] ** 2
                    * self.spec_dat[field]["Zi"] ** 2
                    * Liiprime
    ...
    def calc_species_derivs(self):
        ...
        for spec in self.species:
            for deriv in derivs_in_species_for:
                self.spec_dat[spec][f"d{deriv}dr"] = np.gradient(
                    self.spec_dat[spec][deriv], self.general_dat["r"]
                )

    def calc_general_derivs(self):
        derivs_in_general_for = ["q"]
        for deriv in derivs_in_general_for:
            self.general_dat[f"d{deriv}dr"] = np.gradient(
                self.general_dat[deriv], self.general_dat["r"]
            )
    ...
    def init_dispersion_equation(self):
        ...
        if self.options["mode"] == "KIM":
            if (
                self.options["Collisions"] == "Krook"
                or self.options["Collisions"] == "collisionless"
            ):
        ...
        self.dispersion_model.initialize(
            self.options, self.species, self.spec_dat, self.general_dat, self.equil_dat
        )
    ...
    def find_roots(self, r_ind, contour):
        ...
        try:
            ...
            while roots_number > self.options["number_of_roots_to_find"] or roots_number < 2:
                ...
                if roots_number == 0:
                    contour = cx.Rectangle(
                        np.array(contour.x_range) * search_scale_bigger,
                        np.array(contour.y_range) * search_scale_bigger,
                    )
                if roots_number > self.options["number_of_roots_to_find"]:
                    contour = cx.Rectangle(
                        np.array(contour.x_range) / search_scale_smaller,
                        np.array(contour.y_range) / search_scale_smaller,
                    )
                iteration += 1
                if iteration > 100:
                    print(
                        f"Abort. Too many iterations. Did not find exactly {self.options['number_of_roots_to_find']} roots"
                    )
                    return 1
        except:
    ...
    def calc_needed_susc_funcs(self):
        for spec in self.species:
            self.spec_dat[spec]["x1"] = (
                self.general_dat["kp"] * self.spec_dat[spec]["vT"] / self.spec_dat[spec]["nu"]
            )
            self.spec_dat[spec]["I00"] = {}
            self.spec_dat[spec]["I20"] = {}
            for m_phi in range(0, self.options["max_cyclotron_harmonic"] + 1):
                self.spec_dat[spec]["x2"] = (
                    -(
                        m_phi * self.spec_dat[spec]["omega_c"]
                        + self.general_dat["om_E"]
                        - self.options["omega"]
                    )
                    / self.spec_dat[spec]["nu"]
                )
                self.spec_dat[spec]["I00"][m_phi] = np.zeros(
                    self.general_dat["prof_length"], dtype=complex
                )
                self.spec_dat[spec]["I20"][m_phi] = np.zeros(
                    self.general_dat["prof_length"], dtype=complex
                )
                for i in range(0, self.general_dat["prof_length"]):
                    I = susc_funcs.calc_imn_array(
                        self.spec_dat[spec]["x1"][i], self.spec_dat[spec]["x2"][i]
                    )
            ...
            for key in self.spec_dat[spec]:
                if key == "I00" or key == "I20":
                    for mphi in range(0, self.options["max_cyclotron_harmonic"] + 1):
                        # try:
                        h5f.create_dataset(
                            study + "/" + spec + "/" + key + "/mphi_" + str(mphi),
                            data=self.spec_dat[spec][key][mphi],
                        )
                        # except:
                    ...
                    try:
                        h5f.create_dataset(
                            study + "/" + spec + "/" + key, data=self.spec_dat[spec][key]
                        )
                    except:
    ...
    def plot_bessel_of_mphi(self):
        ...
        for i, mphi in enumerate(mphi_list):
            ...
            plt.plot(
                eval_b, np.imag(Bess0), label=f"Im, Bessel0, mphi = {mphi}", ls=":", marker="."
            )
            plt.plot(
                eval_b, np.imag(Bess1), label=f"Im, Bessel1, mphi = {mphi}", ls=":", marker="."
            )
...
def test_FokkerPlanck(mode, collisions):
    ...
    kwkb.prof_path = (
        "/Users/markusmarkl/plasma/6codes/0projects/2025-test-kim/fort_wkb/runpath/profiles/"
    )
    mphi_max = 0
...
def test_ABC(mode, collisions):
    ...
    study = "ABC_test"
    kwkb.set_output_h5_file(
        f"./" + study + f'_{mode}_{kwkb.options["Collisions"]}.h5', append_or_write="w"
    )
    kwkb.calc_dispersion_relation_k_of_r(mode=mode, collisions=collisions)
