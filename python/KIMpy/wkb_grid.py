...
class WKB_Grid:
    ...
    def generate_non_equidistant_grid(self, start, end, num_points, center, spread):
        ...
        transformed_grid = (transformed_grid - transformed_grid.min()) / (
            transformed_grid.max() - transformed_grid.min()
        )
    ...
    def gen_grid(self, grid, center, spread, num_points):
        ...
        while new_grid[i] < grid_max:
            new_grid.append(
                new_grid[i] + hrmax / (1.0 + 5 * np.exp(-(((new_grid[i] - center) / spread) ** 2)))
            )
            i += 1
