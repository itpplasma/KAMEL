...
class KQ_processor:
    ...
    def process_profiles(self, prof_path, skip=True):
        ...
        if not os.path.exists(self.save_profs + "/r_eff/kprof/k.dat"):
            try:
                ...
                raise ValueError("Error copying kprof to r_eff/kprof")
        ...
        self.pp.map_profs_to_reff(
            self.prof_path + "orig/", self.save_profs + "/r_eff/", self.flux_data, plot=False
        )
    ...
    def get_tMHD_current(
        self,
        curr_file,
        flux_data_path,
        m_mode=np.array([10]),
        delta_phi=0.0,
        coil_curr_scale_l=1.0,
        coil_curr_scale_u=1.0,
        case="standard",
        InputFile="",
        kind="orig",
    ):
        ...
        self.tmhd.mix_coil_rows(
            delta_phi=delta_phi,
            coil_curr_scale_l=coil_curr_scale_l,
            coil_curr_scale_u=coil_curr_scale_u,
        )
    ...
    def prepare_efit2boozer_inp(self):
        ...
        print(
            os.path.abspath(
                os.path.join(
                    os.path.dirname(__file__) + "/../../../efit_to_boozer/RUN/efit_to_boozer.inp"
                )
            )
        )
        ...
        shutil.copy(
            ...
            self.scriptpath + "/efit_to_boozer.inp",
        )
        # get psi max
    ...
    def prepare_field_divB0_inp(self):
        ...
        fp.write_field_divB0_inp(
            os.path.join(
                os.path.dirname(__file__) + "/../../../efit_to_boozer/RUN/field_divB0.inp"
            ),
            self.scriptpath + "/field_divB0.inp",
        )
    ...
    def apply_analytical_criterion(self, m_mode=np.array([6]), n_mode=np.array([2]), Impar=1.0):
        ...
        for i, n in enumerate(n_mode):
            for j, m in enumerate(m_mode):
                ...
                self.crit[j, i] = analytical_local_criterion(
                    m,
                    n,
                    self.pp.r_eff,
                    self.pp.Da,
