...
import numpy as np
from device_config import AUG_config, MASTU_config
...
class KiLCA_interface:
    ...
    EXEC_PATH = os.path.join(
        ...
        "install",
        "bin",
        "KiLCA_Normal_V_2.4.2_MDNO_FPGEN_POLYNOMIAL_Release_64bit",
    )
    ...
    def set_zones(self, r, b, m):
        ...
        self.zones = [
            copy.deepcopy(KiLCA_zone(k, r[k], b[k], m[k], r[k + 1], b[k + 1]))
    ...
    def write(self):
        ...
        if (
            not self.path_of_run
            or path_of_run_stripped == "profiles"
            or os.path.basename(path_of_run_stripped) == "profiles"
        ):
            raise ValueError(
                f"Invalid path_of_run: '{self.path_of_run}'. Must be set to a run directory like 'path/flre/' or 'path/vacuum/'."
            )
        ...
        if resolved_profile_target == resolved_link_full:
            raise ValueError(
                f"Cannot create symlink: source {self.path_of_profiles} resolves to the same path as destination {profile_link_path}. "
        ...
        if os.path.islink(profile_link_path):
            ...
            try:
                ...
                if contents == ["profiles"] and os.path.islink(
                    os.path.join(profile_link_path, "profiles")
                ):
                    ...
                    raise ValueError(
                        f"Cannot create profiles symlink at {profile_link_path}: directory exists with contents. "
                ...
                raise ValueError(
                    f"Cannot safely prepare profiles symlink at {profile_link_path}: {e}"
                ) from e
    ...
    def check_profile_consistency(self, path_profiles=""):
        ...
        for prof_type in req_profs:
            ...
            if count < 3:
                ...
                if np.any(dat[:, 1] < 0):
                    raise ValueError(
                        f"Profile file {prof_type} contains negative values, but it should not!"
                    )
            count = count + 1
