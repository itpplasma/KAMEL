...
def analytical_local_criterion(m, n, r_Da, Da, R0, B0, r, q, Te, Ti, ne, Er, Impar, Zi=1, mi=2):
    ...
    rDe = np.sqrt(
        Te_res * const_eV_to_erg / (4 * np.pi * ne_res * const_e**2)
    )  # Debye length in cm
    kz = n / R0
    ...
    Lee = (
        23.5
        - np.log(np.sqrt(ne_res) / Te_res**1.25)
        - np.sqrt(1e-5 + (np.log(Te_res) - 2.0) ** 2 / 16)
    )
    ...
    VeD = (
        sol / (const_e * ne_res * B0) * np.interp(rm, r, np.gradient(ne * Te * const_eV_to_erg, r))
    )
    # ExB velocity
    VE = sol * Er_res / B0
    ...
    crit4 = (279 * nue**3 + 47 * omega_E**2 * nue) / (
        9 * nue**4 + 10 * omega_E**2 * nue**2 + omega_E**4
    )
    crit5 = np.abs(Impar * sol) ** 2 * kz**2 / (Da_res * B0**2)
    crit = (
        crit1 * crit2 * crit3 * crit4 * crit5
    )  # factor 2 is a spectral weight since the Fourier decomposition of the fields in KiLCA are only over positive integers, but GPEC uses pos and negative integers
...
def crit_from_Br(m, n, r_Da, Da, R0, B0, r, q, Te, Ti, ne, Er, Impar, Br=0, Zi=1, mi=2):
    ...
    crit1 = vTe**2 / 8
    crit2 = (47 * omega_E**2 * nue + 279 * nue**3) / (
        omega_E**4 + 10 * omega_E**2 * nue**2 + 9 * nue**4
    )
