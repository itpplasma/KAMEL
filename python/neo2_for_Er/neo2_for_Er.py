...
class neo2_for_Er:
    ...
    def run_neo2(self, gfile, convex_wall, flux_data):
        ...
        tau = (
            3 * self.pmass**2 * v_th**3 / (16 * np.sqrt(np.pi) * ne_neo * self.echarge**4 * collog)
        )
        kappa = -2 / (v_th * tau)
        ...
        self.fp.write_field_divB0_inp(
            ...
            self.kpath + "TEMPLATE_DIR/field_divB0.inp",
        )
    ...
    def run_remote_neo2(self, configfile, kpath):
        ...
        with open(configfile, "r") as file:
            for line in file:
                ...
                maxjob.append(
                    int(
                        np.min(
                            [np.floor(int(values[2]) / minram), np.floor(int(values[1]) / mincpu)]
                        )
                    )
                )
        ...
        while True:
            ...
            print(
                f"Time elapsed: {time.time() - start_time:.2f} s; Jobs: {len(jobsfinished)} / {totaljobs} finished"
            )
