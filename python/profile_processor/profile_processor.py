...
from scipy.integrate import cumulative_trapezoid, odeint, solve_ivp
...
class Profile_Processor:
    ...
    def run_fieldpy(self, gfile, convex_wall, flux_data, pfile="", skip=False):
        ...
        if not skip:
            ...
            self.fp.write_field_divB0_inp(
                self.fp.path_to_field_divB0_inp + "template_field_divB0.inp",
                self.flux_data + "field_divB0.inp",
            )
    ...
    def extend_kinetic_profiles(self):
        for i, prof in enumerate(self.kin_prof_list):
            self.ext = Profile_Extender(
                self.profile_r_eff_path + prof, self.profile_extended_path + prof, self.factor[i]
            )
            self.ext.read()
    ...
    def extend_q_profile(self):
        ...
        self.Bth_out = (
            ...
            * np.pi
            / self.c
            * cumulative_trapezoid(self.Jz.r_out * self.Jz.y_out, self.Jz.r_out, initial=0.0)
            / self.Jz.r_out
            + Bth_ode[0] * r_ode[0] / self.Jz.r_out
        )
        self.Bz_out = (
            -4.0
            * np.pi
            / self.c
            * cumulative_trapezoid(self.Jth.y_out, self.Jth.r_out, initial=0.0)
            + Bz_ode[0]
        )
        ...
        np.savetxt(
            self.profile_extended_path + "Bth.dat", np.column_stack((self.r_eff, self.Bth_out))
        )
        np.savetxt(
            self.profile_extended_path + "Bz.dat", np.column_stack((self.r_eff, self.Bz_out))
        )
        ...
        self.qp.y_out = (
            -q_out
        )  # minus sign is required for transition of cocos from 3 (EFIT g-file) to 7 (KiLCA: r, theta, z)
        self.qp.write()
        self.q = -q_out
    ...
    def calc_Er_prof(self, recalc=False):
        ...
        self.Er = (
            self.Ti * self.eV_to_erg * self.dne / (self.echarge * self.ne)
            + (1.0 - self.k) * self.dTi / self.echarge
    ...
    def collect_k_profile(self, kpath, fname):
        ...
        for j, con in enumerate(content):
            ...
            if (
                os.path.exists(con)
                and os.path.isdir(con)
                and not os.path.basename(con) == "TEMPLATE_DIR"
            ):
                try:
    ...
    def plot_Er_profile(self, save=False):
        ...
        for i, m in enumerate(self.m_mode):
            ...
            plt.text(
                res[i],
                plt.ylim()[1] - 0.03 - np.mod(i, 3) * 0.025,
                f"{self.m_mode[i]}",
    ...
    def plot_kinetic_profs(self, save=False):
        ...
        list(
            map(
                lambda x: x.tick_params(
                    axis="both", which="both", direction="out", top=True, right=True
                ),
                ax[:, 0],
            )
        )
        ...
        for i, m in enumerate(self.m_mode):
            ...
            list(
                map(
                    lambda x: x.text(
                        res[i],
                        x.get_ylim()[1] * (1.0 - np.mod(i, 3) * 0.25),
                        f"{self.m_mode[i]}",
                        horizontalalignment="center",
                    ),
                    ax[:, 0],
        ...
        if save:
            plt.savefig(
                self.plot_dir + "kinetic_of_reff.pdf", transparent=False, bbox_inches="tight"
            )
        plt.show()
