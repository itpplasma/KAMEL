...
class da_estimator:
    ...
    def __init__(self, profpath, fluxdatapath):
        ...
        if not os.path.exists(profpath):
            raise ValueError("Profilepath not found in " + profpath)
        if not os.path.exists(fluxdatapath):
            raise ValueError("Fluxdatapath not found in " + fluxdatapath)
    ...
    def load_estimation(self, shot, time, path, combine):
        ...
        if os.path.exists(astraname):
            ...
            rho_pol = self.dat[:, 2]
            self.chi_e = (
                inter.pchip_interpolate(rho_pol**2, self.dat[:, 3], self.psi_pol_norm) * 1e4
            )
            self.Q_e = (
                inter.pchip_interpolate(rho_pol**2, self.dat[:, 5], self.psi_pol_norm)
                * 1e6
                / self.e
            )
    ...
    def export2hdf5(self, fname):
        ...
        self.writehdf5(
            ...
            "/Da_est/dTe",
            self.dTe,
            "radial derivative of the electron temperature profile",
            "eV cm^{-1}",
        )
        self.writehdf5(
            fname, "/Da_est/Da", self.Da, "anomalous diffusion coefficient", "cm^2 s^{-1}"
        )
        ...
        if not (self.Prad is None):
            self.writehdf5(
                fname, "/Da_est/Prad", self.Prad, "radiated power in plasma", "eV s^{-1}"
            )
        if not (self.Peff is None):
            self.writehdf5(
                fname, "/Da_est/Peff", self.Pinp, "effective power on electrons", "eV s^{-1}"
            )
        if not (self.chi_e is None):
            self.writehdf5(
                fname, "/Da_est/chi_e", self.chi_e, "astra chi of electrons", "cm^2 s^{-1}"
            )
