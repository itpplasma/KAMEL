...
from libneo.boozer import (
    get_B0_of_s_theta_boozer,
...
class tMHD_current:
    ...
    def load_curr_harmonics_MARSF(self, file):
        ...
        self.JparU_harm = (
            mat["JparU_harm[A/m^2]"] * self.Apermsq_to_statApercmsq
        )  # conversion from A/m^2 to statA/cm^2
    ...
    def loadCurrentMARSFSepDensScan(self, file, kind, InputFile):
        ...
        if "Scan" in file:
            kinds = ["X1", "X2", "X3", "X4", "X5", "X6", "X7", "X8", "X9", "X10"]
        else:
            kinds = ["orig", "smooth"]
        ...
        if kind == "orig" or kind == "smooth":
            ...
            self.JparU = (
                np.array(dat["Jpars"][kind][0][0]["UPPER"][0][0], dtype=complex)
                * self.Apermsq_to_statApercmsq
            )
            # self.JparL = np.array(dat.get('Jpars/'+kind+'/LOWER'), dtype=complex) / 10**5
            self.JparL = (
                np.array(dat["Jpars"][kind][0][0]["LOWER"][0][0], dtype=complex)
                * self.Apermsq_to_statApercmsq
            )
    ...
    def loadCurrentMARSFStandard(self, file):
        mat = loadmat(file)
        self.JparU = (
            mat["JparU[A/m^2]"] * self.Apermsq_to_statApercmsq
        )  # conversion from A/m^2 to statA/cm^2
    ...
    def mix_coil_rows(self, delta_phi=0.0, coil_curr_scale_l=1.6, coil_curr_scale_u=1.5):
        ...
        if hasattr(self, "JparU_harm"):
            self.Jpar_harm = (
                coil_curr_scale_u * self.JparU_harm
                + coil_curr_scale_l * self.JparL_harm * np.exp(-1j * delta_phi)
            )
        if hasattr(self, "JparU"):
            self.Jpar = coil_curr_scale_u * self.JparU + coil_curr_scale_l * self.JparL * np.exp(
                -1j * delta_phi
            )
    ...
    def make_Jpar_periodic(self):
        for js in range(0, len(self.s_equil)):
            if np.abs(self.Jpar[js, 0] - self.Jpar[js, -1]) > 1e-3:
                raise ValueError(
                    f"js = {js}, curr[0] = {curr[js,0]}, curr[-1] = {curr[js,-1]}, diff = {np.abs(curr[js,0] - curr[js,-1])}"
                )
    ...
    def get_Jpar_over_B0_boozer_harmonics(self, n=2):
        ...
        def fun_Jpar(ind_stor, theta):
            ...
            for i, _ in enumerate(theta):
                Jpar_of_s.append(
                    CubicSpline(self.chi, self.Jpar[ind_stor, :], bc_type="periodic")(theta[i])
                )
            Jpar_of_s = np.array(Jpar_of_s)
            return Jpar_of_s

        self.Jpar_over_B0_harm = get_boozer_harmonics_divide_f_by_B0_1D_fft(
            fun_Jpar, self.stor, self.nth, n
        )
    ...
    def integrate_curr_dens(self, m_mode=np.array([10])):
        ...
        for i, m in enumerate(m_mode):
            ...
            self.current_m[i, :] = (
                integrate.cumtrapz(curr_dens_m * self.stor[:-1], self.stor[:-1], initial=0)
                * 2.0
                * self.psi_tor[-1]
                * 2.0
                * np.pi
            )  # the equil r q psi psi tor is the *small* psi not the large one which is used in the formulation of the integration, hence the factor 2pi
            self.current_p[i, :] = (
                integrate.cumtrapz(curr_dens_p * self.stor[:-1], self.stor[:-1], initial=0)
                * 2.0
                * self.psi_tor[-1]
                * 2.0
                * np.pi
