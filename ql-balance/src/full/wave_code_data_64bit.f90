...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Edited: 19.02.2021 by Markus Markl
...
! Read in profiles, where all profiles have the same dimension!!
    ...
    CALL h5_get_double_1(group_id_1, "q", iq)
    CALL h5_get_double_1(group_id_1, "n", in)
    CALL h5_get_double_1(group_id_1, "Ti", iTi)
    CALL h5_get_double_1(group_id_1, "Te", iTe)
    CALL h5_get_double_1(group_id_1, "Vth", iVth)
    CALL h5_get_double_1(group_id_1, "Vz", iVz)
    CALL h5_get_double_1(group_id_1, "Er", idPhi0)

    ! get r values (are the same for the profiles (?))
    allocate (rq(ub))
    allocate (rn(ub))
    allocate (rTi(ub))
    allocate (rTe(ub))
    ...
    CALL h5_get_double_1(group_id_1, "r_out", rq)
    rn = rq
    rTi = rq
    rTe = rq
    rVth = rq
...
! then be used for linear parameter studies.
!
