module cut_off_integration
    ...
    use kernel, only: K_rho_phi_of_rg, K_rho_phi_llp
    ...
    subroutine basis_transformation_integration(write_out)
        ...
        write(*,*) 'Total elements of kernel array in l space: ', l_space_dim**2
        write(*,*) 'Elements to calculate: ', count_elem_to_calc
        ...
        do i_krp = 1, k_space_dim
            do i_kr = 1, k_space_dim
                do i_rg = 1, npoib
                    ...
                    !call updateLoadingBar(count_loading, total_count_loading)
                end do
            end do
        end do
        !$OMP END PARALLEL DO
            ...
            !if (kr_cutoff - abs(kr(i_kr)) .ge. 0.0d0) then
                ...
                func_trapz_int_2D = func_trapz_int_2D + 0.5d0 * com_unit / (rb(i_rg) - xl(l)) &
                    ...
                    * K_rho_phi_of_rg(i_kr, closest_kr_ind_upper, i_rg) &
                    - varphi_lkr(closest_kr_ind_lower, l) * exp(com_unit * krp(i_kr-1) * (xl(lp) - rb(i_rg))) &
                ...
                func_trapz_int_2D = func_trapz_int_2D + 0.5d0 * com_unit / (xl(lp) - rb(i_rg)) &
                    ...
                    * K_rho_phi_of_rg(closest_kr_ind_upper, i_kr, i_rg) &
                    - conjg(varphi_lkr(closest_kr_ind_lower, lp)) * exp(com_unit * kr(i_kr-1) * (rb(i_rg) - xl(lp))) &
