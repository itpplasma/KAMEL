...
%Runs to make
		shot = 33353; %or 33353 2900ms
time = 2900;  %or 33120 5500ms
profoffset = 0;
equioffset = 0;
...
studyname = ['BALANCE_4SHOTS', proftype];
basepath = '/temp/markl_m/BALANCE_4CASES/';
datapath = [basepath, 'DATA/'];
path2preh5 = [basepath,'PRERUNDATA/',num2str(shot), '/', num2str(shot), '_', num2str(time), '_mi_', num2str(ion_mass),proftype '.hdf5'];

% use copyinitial to use existing profiles to a shot/time.
%copyinitial = ['/temp/ulbl_p/BALANCE_4CASES/RUNS/4Cases_ULBLP_rho_pol/33353_2900/timeevol/profiles/'];
...
%input
gfile  = [datapath, 'EQUI/', num2str(shot),'/g',num2str(shot),'.',num2str(time+equioffset),'_MICDU_EQB_ed1'];
...
system(['mkdir -p ', basepath, 'POST_h5/', studyname, '/', shotname, '/']);
...
disp(['Finished! :), shot: ', num2str(shot), ' at time: ', num2str(time), ' with ion mass= ', num2str(ion_mass)]);
