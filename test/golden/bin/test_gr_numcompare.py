...
def _run(a, b, *extra):
    r = subprocess.run(
        [sys.executable, str(CMP), str(a), str(b), *extra], capture_output=True, text=True
    )
...
def test_nested_numeric_match(tmp_path):
    for root in ("ref", "cur"):
        _write(tmp_path / root / "linear-data" / "m_6_n_2" / "EB.dat", "1.0 2.0\n3.0 4.0\n")
...
def test_near_zero_noise_not_flagged(tmp_path):
    ...
    _write(tmp_path / "ref" / "fields" / "rho.dat", "9.5780e1\n3.1738e-6\n3.9e-12\n")
    _write(tmp_path / "cur" / "fields" / "rho.dat", "9.5780e1\n3.1738e-6\n5.5e-12\n")
    rc, out = _run(tmp_path / "ref", tmp_path / "cur")
    assert rc == 0, out
    assert "FAIL" not in out
...
def test_poy_test_err_not_relatively_compared_across_backends(tmp_path):
    ...
    _write(
        tmp_path / "ref" / "linear-data" / "zone_0_poy_test_err.dat",
        "5.00e1\t5.0e-7\n5.10e1\t3.0e-7\n",
    )
    _write(
        tmp_path / "cur" / "linear-data" / "zone_0_poy_test_err.dat",
        "5.00e1\t5.4e-7\n5.10e1\t3.6e-7\n",
