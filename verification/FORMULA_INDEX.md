...
### Charge-potential kernel oracle (#196)

`verification/mathematica/04_charge_potential_kernels.wl` starts from the
radial Fourier and gyro-phase representations (13.1)–(13.8), independently
integrates the Maxwellian perpendicular-velocity moments, and combines them
with the direct FP response integral from (5.13)–(5.15). Differentiating the
upstream Weber integral proves that the energy moment is
`exp(-b_+) [(1-b_+-m_phi) I_m(b_x) + b_x I_(m-1)(b_x)]`; therefore the
`1-b_+` code sign at `m_phi=0` is correct and the plus sign printed in
(14.3)–(14.4) is a thesis discrepancy. The script separately checks the Krook
...
approximation. `test_flr2_fourier_kernel` consumes all 13 rows and currently
agrees within `1.90e-16` scaled error; the independent script passes 52 checks
